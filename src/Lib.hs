...
instance Policy FIFO where
  update (FIFOList fifoList) id | List.elem id fifoList = FIFOList fifoList
...
instance Policy LFU where
  ...
  evict (LFUHeap lfuHeap) id =
    (evicted, LFUHeap $ Heap.insert (1, id) $ Heap.drop 1 lfuHeap)
    where [(_, evicted)] = Heap.take 1 lfuHeap

instance Policy DLFU where
  update (DLFUHeap dlfuHeap) id
    | length idList == 1 = DLFUHeap
    $ Heap.insert (count, (totalT, totalT, id)) updatedHeap
   ...
   where
    ...
    (idList, otherList) =
      List.partition (\(_, (_, _, val)) -> val == id) heapList
    [(p, (lastT, totalT, _))] = idList
    count = p * const / (const + fromIntegral (totalT - lastT))
    updatedHeap = Heap.fromList otherList

  evict (DLFUHeap dlfuHeap) id =
    (evicted, DLFUHeap $ Heap.insert (1.0, (1, 1, id)) $ Heap.drop 1 dlfuHeap)
